...
@csrf
            @foreach ($dynamic_forms as $dynamic_form)
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-group d-flex">
                            @foreach ($dynamic_form->options as $option)
                                <input type="radio"
                                    name="{{ Illuminate\Support\Str::slug($dynamic_form->dynamic_field, '_') }}" value="{{$option->value}}" @if ($dynamic_form->required)
                                    required
                                @endif>
                ...
                @elseif($dynamic_form->type == 'checkbox')
                    <div class="form-group">
                        ...
                        <div class="form-group d-flex">
                            @foreach ($dynamic_form->options as $option)
                                <input type="checkbox"
                                    name="{{ Illuminate\Support\Str::slug($dynamic_form->dynamic_field, '_') }}[]" value="{{$option->value}}" >
                                <label for="" class="mt-2 mx-3">{{ $option->value }}</label>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="form-group">
                    <label for="" class="my-0">{{ $dynamic_form->dynamic_field ?? '' }}</label>
                    <input type="number" placeholder="e.g 1220" name="{{ Illuminate\Support\Str::slug($dynamic_form->dynamic_field, '_') }}"
                        class="form-control" @if ($dynamic_form->required)
                        required
                    @endif>
                </div>
